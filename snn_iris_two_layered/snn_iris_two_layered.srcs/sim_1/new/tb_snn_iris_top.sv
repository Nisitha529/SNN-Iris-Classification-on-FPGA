...
module tb_snn_iris_top;
    ...
    parameter SAMPLE_FILE     = "/media/nisitha/My_Passport/MOODLE/Vivado_projects/SNN_project/snn_iris_project/snn_iris_project/python_src/iris_test_vectors.txt";  // Use relative path
    ...
    // Outputs
    wire [2:0]  class_out;        // 3-bit one-hot encoded output
    wire        result_valid;
    
    // Testbench Variables
    reg  [7:0]  test_data [0:3];  // Feature storage
    reg  [1:0]  expected_class;   // Expected class (0, 1, or 2)
    integer     file_handle;
    ...
    task load_and_process_samples();
        begin
            ...
            while (sample_count < NUM_TEST_SAMPLES) begin
                ...
                if (class_out == (3'b1 << expected_class))
                    correct++;
