module snn_iris_top (
    ...
    // Parameters - adjusted for two-layer architecture
    localparam TIME_STEPS      = 25;  // Matches SNN model
    localparam PIPELINE_DELAY  = 4;   // Reduced for simpler design
    ...
    // Layer outputs
    wire [7:0]  spikes_layer1;  // First hidden layer (8 neurons)
    wire [2:0]  spikes_layer2;  // Output layer (3 neurons)
    ...
    // Control module - simplified for two layers
    control_iris #(
    ...
    // Output layer: 8-to-3 (directly connected to classification)
    layer2_8to3 layer2 (
        .clk          (clk),
        .reset        (reset),
    ...
    // Spike counters - only for output layer
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            count0 <= 0;
            count1 <= 0;
            ...
            count0 <= count0 + spikes_layer2[0];
            count1 <= count1 + spikes_layer2[1];
            count2 <= count2 + spikes_layer2[2];
        end
    ...
    // Capture final counts at end of processing
